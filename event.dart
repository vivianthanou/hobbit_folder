...
class _EventState extends State<Event> {
            ...
            IconButton(
              ...
              onPressed: () {
                Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) => Event(id: widget.id)));
              },
            ),
            IconButton(
              color: const Color.fromARGB(255, 253, 110, 0),
  ...
  Future _defaultImage(String cat) async {
    if (cat == 'One') {
      ...
      setState(() {
        _selectedImage = const Image(
            image: NetworkImage(
                'https://icons.veryicon.com/png/o/sport/motion/skate-9.png'));
      });
    } else if (cat == 'Five') {
      setState(() {
        _selectedImage = const Image(
  ...
  Future _pickImageFromGallery() async {
    ...
    if (pickedFile != null) {
      setState(() {
        ...
        _selectedImage =
            Image.file(File(pickedFile.path)); // Update the image for display
      });
    }
  }
  ...
  void _createEvent(BuildContext context) async {
    ...
    var request = http.MultipartRequest(
        'POST', Uri.parse('http://192.168.150.2:5000/create_event'));
    ...
    final url = Uri.parse('http://192.168.150.2:5000/create_event');
